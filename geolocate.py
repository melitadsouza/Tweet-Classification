#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# The code for predicting the 5 words for the 12 cities doesn't take that long,
# but creation of output file and accuracy calculaton takes a really long time(around 20-30mins) on the SOIC server.
# It is probably due to the use of dataframes.
# Current accuracy obtained is 66% but there is a slight improvement, when repeated words in all 12 cities are eliminated
# as they do not help in distinguishing the class of the tweet.
# When custom words like ['Job', 'hiring', 'jobs', 'careerarc', 'latest', 'opening'] are eliminated with the usual english stopwords,
# accuracy obtained is 67.5 %, and the time taken is reduced by 2-3 mins.
        ...
        for index, row in self.test_df.iterrows():
            ...
            if(print_top_five):    
                for key, val in likelihoods.items():
                    sorted_likelihoods = sorted(val.items(), key=operator.itemgetter(1), reverse = True)[0:5]
